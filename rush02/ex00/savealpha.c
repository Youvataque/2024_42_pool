...
/*   Updated: 2024/08/04 18:34:34 by yseguin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_is_ten(char *str, t_dic *tab, int len)
{
	int	j;
...
void	ft_is_unit(char *str, t_dic *tab, int len)
{
	int	j;
...
void	ft_print(char *str, t_dic *tab)
{
	...
	len = ft_strlen(str);
	if (*str == '0' && len == 1)
		return (ft_is_zero(str, tab));
	if (len == 1)
	{
		ft_is_unit(str, tab, len);
		write (1, "\n", 1);
		return ;
	}
	while (len > 0)
	{
		...
		if (len % 3 == 2)
			ft_is_ten(str, tab, len);
		if (len % 3 == 1 && str[-1] != '1')
			ft_is_unit(str, tab, len);
		if (len > 3 && len % 3 == 1)
			ft_is_big(str, tab, len);
		ft_margoulin(&len, &str);
	}
	write(1, "\n", 1);
}
